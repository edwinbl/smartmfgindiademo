    ...
    <div ref={formRef} className={cn("cii-card p-6 sm:p-8", !embedded && "max-w-3xl mx-auto sm:p-10")}>
          ...
          {submitted ? (
            <div className="mt-8 p-6 rounded-md border text-center" style={{ borderColor: "hsl(var(--india-green) / 0.4)", background: "hsl(var(--india-green) / 0.06)" }}>
              <h3 className="font-display font-bold text-lg text-[hsl(var(--navy-900))]">Thanks — your request is in.</h3>
              <p className="text-sm text-[hsl(var(--neutral-700))] mt-1">A member of our team will be in touch within 1–2 business days.</p>
              <button onClick={() => setSubmitted(false)} className="link-arrow mt-4">Submit another request →</button>
            </div>
          ) : (
            <form onSubmit={onSubmit} className="mt-8 space-y-5">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Share a brief note about what you're looking for…"
                  className={cn(
        ...
        <div className="sm:col-span-2">
          ...
          >
            ...
            <span className="text-sm text-[hsl(var(--neutral-700))]">
              {fileName ? fileName : "Click to upload — PNG, JPG or PDF up to 10MB"}
            </span>
            <input
              id="support-file"
              type="file"
